...
import type { ContentStatus, IndustryDetail, LocaleCode } from '@/types';
...
export default function IndustryForm({ industry, statuses, mode, canManageContent = true, locale }: Props) {
    const currentLocale = locale.current;
    const isDeleted = Boolean(industry?.deleted_at);
    ...
    function submit(event: FormEvent<HTMLFormElement>) {
        ...
        if (mode === 'create') {
            form.post('/admin/industries', { preserveScroll: true });
            return;
        }

        if (industry) {
            form.patch(`/admin/industries/${industry.id}`, { preserveScroll: true });
        }
    }

    function destroy() {
        if (industry && confirm(currentLocale === 'ar' ? 'هل تريد حذف هذا القطاع؟' : 'Delete this industry?')) {
            router.delete(`/admin/industries/${industry.id}`, { preserveScroll: true });
        }
    }

    function restore() {
        if (industry) {
            router.patch(`/admin/industries/${industry.id}/restore`, {}, { preserveScroll: true });
        }
    }

    return (
        <>
            <Head title={mode === 'create' ? 'Create Industry' : `Edit Industry - ${industry?.title_en ?? ''}`} />
            <div className="space-y-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <Link href="/admin/industries" className="inline-flex items-center gap-2 text-sm font-bold text-[#0AA6B5]">
                            <ArrowLeft className="size-4" />
                            {currentLocale === 'ar' ? 'العودة إلى القطاعات' : 'Back to industries'}
                        </Link>
                        <div className="mt-4 flex flex-wrap items-center gap-3">
                            ...
                            {industry ? <StatusBadge status={industry.status} /> : null}
                    ...
                    <Card className="rounded-lg border-[#D8E2EC] bg-white">
                        <CardHeader>
                            <CardTitle className="text-lg font-extrabold text-[#0F172A]">
                                {currentLocale === 'ar' ? 'محتوى القطاع' : 'Industry content'}
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="grid gap-5">
                            ...
                            <div className="grid gap-5 md:grid-cols-2">
                                <label className="grid gap-2">
                                    <span className="text-sm font-bold text-[#0F172A]">{currentLocale === 'ar' ? 'الوصف بالإنجليزية' : 'English description'}</span>
                                    <textarea
                        ...
                        <CardContent className="space-y-5">
                            ...
                            <label className="grid gap-2">
                                ...
                                <input
                                    ...
                                    disabled={!canEdit}
                                    placeholder="images/industries/example.jpg"
                                    className="h-10 rounded-md border border-[#D8E2EC] bg-white px-3 font-mono text-sm outline-none focus:border-[#0AA6B5] focus:ring-3 focus:ring-[#22C7CF]/25 disabled:opacity-70"
                                />
